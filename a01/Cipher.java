...
public class Cipher {
    ...
    public static String shift(final String s, final int n) {
    	...
    	for (int i = 0; i < upperCaseString.length(); i++) {
    		...
    		int currentIndex = ALPHA.indexOf(currentChar);
    		
    		if (currentIndex != -1) {
    			int newIndex = (currentIndex + n) % ALPHA_LENGTH;
    			
    			result = result + (ALPHA.charAt(newIndex));
	...
	for (int i = 0; i < upperCaseString.length(); i++) {
		...
		if (currentIndex != -1) {
			char cipherChar = ciphertext.charAt(currentIndex);
