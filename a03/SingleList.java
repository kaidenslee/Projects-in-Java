...
public class SingleList<T extends Comparable<T>> extends SingleLink<T> {
    ...
    public void append(final T data) {
    	...
    	SingleList<T> temp = new SingleList<>();
        temp.front = newNode;
        temp.rear = newNode;
        temp.length = 1;
	...
	while(current != null) {
		...
		while(runner.getNext() != null) {
			if (runner.getNext().getObject().equals(current.getObject())) {
	...
	while(current != null) {
		boolean result = current.getObject().equals(key);
		if(result) {
			count++;
		}
		current = current.getNext();
    ...
    public T find(final T key) {
        ...
        return result.getNext().getObject();
    }
	...
	while(curr1 != null) {
		if(!(curr1.getObject().equals(curr2.getObject()))) {
			return false;
		}
		curr1 = curr1.getNext();
		curr2 = curr2.getNext();
    ...
    while (currentLeft != null) {
        T value = currentLeft.getObject();
        
        if (right.contains(value) && !this.contains(value)) {
            SingleNode<T> newNode = new SingleNode<>(value, null);
